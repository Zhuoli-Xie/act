...
class DETRVAE(nn.Module):
    ...
    def forward(self, image, env_state, actions=None, is_pad=None):
        """
        qpos: batch, qpos_dim
        ...
        if is_training:
            ...
            cls_joint_is_pad = torch.full((bs, 1), False).to(actions.device) # False: not a padding
            ...
            mu = logvar = None
            latent_sample = torch.zeros([bs, self.latent_dim], dtype=torch.float32).to(actions.device)
            latent_input = self.latent_out_proj(latent_sample)
        ...
        if self.backbones is not None:
            ...
            transformer_input = env_state # seq length = 2
